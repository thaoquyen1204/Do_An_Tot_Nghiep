...
class TrucquanDG extends StatefulWidget {
  ...
  final String? photoURL;
     TrucquanDG({required this.userEmail, required this.displayName, this.photoURL,});  // Thêm tham số cho năm
...
Widget build(BuildContext context) {
  ...
  int approvedCount = statusMap.values.where((status) => status == 'Chưa đánh giá').length;
  int rejectedCount = statusMap.values.where((status) => status == 'Chưa đủ điều kiện').length;
  int pendingCount = statusMap.values.where((status) => status == 'Đã được đánh giá').length;
                    ...
                    Expanded(
                      child: isLoading
                          ...
                          : SingleChildScrollView(
                              child: buildCharts(approvedCount, rejectedCount, pendingCount,unknownCount),
                            ),
                    ),
                  ],
                ),
...
Widget buildDrawer() {
  return Column(
    children: [
      ...
      ...["KPI", "Tài liệu", "Thông báo", "Thông tin cá nhân", "Cài đặt", "Xem biểu mẫu"].map((title) {
      ...
      ElevatedButton(
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => DanhsachCNDG(userEmail: widget.userEmail, displayName: widget.displayName,photoURL:widget.photoURL ,),
            ),
          );
        },
        child: Text('Quay lại'),
...
Widget buildCharts(int approvedCount, int rejectedCount, int pendingCount, int unknownCount,) {
  return Column(
    children: [
              ...
              bottomTitles: AxisTitles(
                sideTitles: SideTitles(
                  ...
                  getTitlesWidget: (double value, TitleMeta meta) {
                    switch (value.toInt()) {
                      case 0:
                        return Text('Chưa đánh giá', style: TextStyle(fontSize: 10));
                      case 1:
                        return Text('Chưa đủ điều kiện', style: TextStyle(fontSize: 10));
                      case 2:
                        return Text('Đã được đánh giá', style: TextStyle(fontSize: 10));
                      case 3:
                        return Text('Không xác định', style: TextStyle(fontSize: 10));
                ...
                sideTitles: SideTitles(
                  ...
                  getTitlesWidget: (value, meta) {
                    switch (value.toInt()) {
                      ...
                      case 1:
                        return Text('Chưa tạo', style: TextStyle(fontSize: 10));
                      default:
                        return Text('');
                    }
                  },
