...
class Usuario_provider1 with ChangeNotifier {
  ...
  Future<void> updateKPIBenhVien(Kpibenhvien kpibenhvien) async {
    ...
    final resp = await http.put(
      ...
      body: json.encode(kpibenhvien.toJson()), // Chuyển đổi đối tượng thành JSON
    );
...
String? getMaphieuKpiBvByNam(int nam) {
  ...
  try {
    final Kpibenhvien = kpibenhvien.firstWhere((user) => user.namphieu == nam);

    if (Kpibenhvien.maphieukpibenhvien != null && Kpibenhvien.maphieukpibenhvien!.isNotEmpty) {
...
//Chi tiết tieu chí mục tiêu bẹnh viện
 ...
 ChiTietKPIBVByNam(int year) async {
  final url = Uri.http(urlapi, 'api/ChiTietTieuChiMucTieuBVs/GetChiTietKPIBVByNam', {
    'Nam': year.toString(),
  });
  ...
  if (resp.statusCode == 200) {
    try {
      ...
      if (responseData.isEmpty) {
        throw Exception('Dữ liệu không tồn tại cho mã khoa phòng và năm này.');
      }
    ...
    throw Exception('Không thể tải dữ liệu KPI. Mã trạng thái: ${resp.statusCode}, Nội dung phản hồi: ${resp.body}');
  }
}

Future<void> deletecchitietKPIBenhVien(String maphieukpibenhvien) async {
  final url1 = Uri.http(urlapi, 'api/ChiTietTieuChiMucTieuBVs/$maphieukpibenhvien');
...
Future<void> updateKPICaNhan(Kpicanhan kpicanhan) async {
  final url1 = Uri.http(urlapi, 'api/KPI_CaNhan/${kpicanhan.maphieukpicanhan}');
  ...
  try {
    ...
    if (resp.statusCode == 204) {
      ...
      throw Exception('Thất bại cập nhật KPI cá nhân, mã lỗi: ${resp.statusCode}');
    }
  } catch (e) {
    print('Lỗi xảy ra: $e');
    rethrow;
 ...
 Future<Kpicanhan?> getKpicanhanByMaphieu(String maPhieu) async {
  final url = Uri.http(urlapi, 'api/KPI_CaNhan/getKpicanhanByMaphieu', {
    'maPhieu': maPhieu
  });
...
String? getMaphieuKpCNByNamandMaNV(int nam, String manv) {
  ...
  try {
    final Kpicanhan = kpicanhan.firstWhere((user) => user.quynam == nam && user.manv ==manv);

    if (Kpicanhan.maphieukpicanhan != null && Kpicanhan.maphieukpicanhan!.isNotEmpty) {
...
Future<void> ChiTietKPICNByMaNVvaNam(String maNV, int year) async {
  final url = Uri.http(urlapi, 'api/ChiTietKPICaNhanNVs/GetChiTietKPICNByMaNVvaNam', {
    'maNV': maNV,
    'Nam': year.toString(),
  });
  ...
  if (resp.statusCode == 200) {
    try {
      ...
      // Nếu API trả về một danh sách rỗng (không có dữ liệu cho năm/mã NV)
      if (responseData.isEmpty) {
        throw Exception('Dữ liệu không tồn tại cho mã nhân viên và năm này1.');
      }
...
Future<void> ChiTietKPICNByMaNVvaNamAll(String maNV, int year) async {
  final url = Uri.http(urlapi, 'api/ChiTietKPICaNhanNVs/GetChiTietKPICNByMaNVvaNamAll', {
    'maNV': maNV,
    'Nam': year.toString(),
  });
  ...
  if (resp.statusCode == 200) {
    try {
      ...
      // Nếu API trả về một danh sách rỗng (không có dữ liệu cho năm/mã NV)
      if (responseData.isEmpty) {
        throw Exception('Dữ liệu không tồn tại cho mã nhân viên và năm này3.');
      }
...
Future<double> GetTyLeHoanThanhByMaNVvaNam(String maNV, int year) async {
  final url = Uri.http(urlapi, 'api/ChiTietKPICaNhanNVs/GetTyLeHoanThanhByMaNVvaNam', {
    'maNV': maNV,
    'Nam': year.toString(),
  });
  ...
  try {
    ...
    if (resp.statusCode == 200) {
      double tylehoanthanh = json.decode(resp.body); // Phản hồi là số nguyên
      return tylehoanthanh; // Trả về giá trị trạng thái nhận được từ API
    } else if (resp.statusCode == 404) {
      return -1; // Nếu không tìm thấy thì trả về -1
    } else {
      throw Exception('Không thể tải chi tiết KPI. Mã trạng thái: ${resp.statusCode}');
    ...
    throw Exception('Đã xảy ra lỗi trong khi lấy trạng thái KPI9.');
  }
...
Future<void> updateChiTietKPICaNhan(ChiTietKPICaNhanNV chitietkpicanhan) async {
  final url1 = Uri.http(
    urlapi,
 ...
 Future<ChiTietKPICaNhanNV?> getChiTietKpicanhanByMaphieu(int stt) async {
  final url = Uri.http(urlapi, 'api/ChiTietKPICaNhanNVs/getChitietKpicanhanByMaphieu', {
    'stt': stt.toString(),
  });
  print('Constructed URL: $url');
...
Future<int> GetidByMaPhieuandMaKPI(String maPhieu, int MaKPI) async {
  final url = Uri.http(urlapi, 'api/ChiTietKPICaNhanNVs/GetidByMaPhieuandMaKPI', {
    'maPhieu': maPhieu,
