...
class _TestState extends State<XemTTNguoidung> {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        // Kiểm tra nếu màn hình rộng nhỏ hơn 600px
        ...
        return Scaffold(
          ...
          drawer: isSmallScreen
              ? _buildDrawer1(
                  context) // Ẩn SideMenu trên màn hình nhỏ nếu không cần thiết
              : _buildDrawer(context), // Hiển thị SideMenu trên màn hình lớn
          body: Padding(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildInfoCard(
                Icons.phone, nguoidung!.sdt ?? 'Chưa có số điện thoại'),
            _buildInfoCard(Icons.email, nguoidung!.gmail ?? 'Chưa có gmail'),
            _buildInfoCard(Icons.account_circle,
                nguoidung!.tenTaiKhoan ?? 'Chưa có tên tài khoản'),
            _buildPasswordCard(
                Icons.lock, nguoidung!.matKhau ?? 'Chưa có mật khẩu'),
            _buildInfoCard(Icons.work_outline,
                nguoidung!.tenChucDanh ?? 'Chưa có chức danh'),
            _buildInfoCard(
                Icons.security, nguoidung!.tenQuyen ?? 'Chưa có quyền'),
          ],
        ),
      ),
    );
            ...
            children: [
              ...
              IconButton(
                ...
                onPressed: () {
                  setState(() {
                    _obscurePassword =
                        !_obscurePassword; // Thay đổi trạng thái ẩn/hiện
                  });
                },
              ),
            ],
  ...
  Widget _buildDrawer1(BuildContext context) {
    return Container(
