...
class DanhsachBieuMauCaNhan extends StatefulWidget {
  ...
  @override
  _DanhsachBieuMauCaNhanGridScreenState createState() =>
      _DanhsachBieuMauCaNhanGridScreenState();
}

class _DanhsachBieuMauCaNhanGridScreenState
    extends State<DanhsachBieuMauCaNhan> {
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            SizedBox(
                                width:
                                    8), // Khoảng cách giữa chữ "Năm:" và TextField
                            Container(
                                ...
                                onSubmitted: (value) {
                                  ...
                                  setState(() {
                                    currentYear = int.tryParse(value) ??
                                        currentYear; // Cập nhật năm
                                  });
                                },
                              ),
                            ),
                            SizedBox(
                                width: 16), // Khoảng cách giữa TextField và nút
                            ElevatedButton(
    ...
    List<charts.Series<ChartData, String>> series = [
      charts.Series(
        ...
        domainFn: (ChartData data, _) =>
            _shortenChucDanhName(data.tenChucDanh), // Hiển thị tên cắt ngắn
        measureFn: (ChartData data, _) =>
            data.trangThaiIndex, // Giá trị trạng thái
        ...
        labelAccessorFn: (ChartData row, _) =>
            '${_mapStatus(row.trangThaiIndex)}', // Nhãn cho thanh
      )
    ];
    ...
    return Container(
      ...
      child: charts.BarChart(
        ...
        barRendererDecorator:
            charts.BarLabelDecorator<String>(), // Hiển thị nhãn trên thanh
        ...
        behaviors: [
          ...
          charts
              .DomainHighlighter(), // Hiển thị đường kẻ khi rê chuột qua thanh
        ],
      ),
    );
  }

  List<ChucDanh> _removeDuplicateChucDanhs(List<ChucDanh> chucDanhs) {
    final seen = <String>{}; // Bộ nhớ lưu trữ mã chức danh đã thấy
    return chucDanhs.where((chucDanh) {
      return seen.add(chucDanh.maChucDanh); // Chỉ thêm khi mã chưa tồn tại
    }).toList();
  }

  List<ChartData> _createChartData() {
    List<ChucDanh> uniqueChucDanhs = _removeDuplicateChucDanhs(chucDanhs);

    return uniqueChucDanhs.map((chucDanh) {
      String status = statusMap[chucDanh.maChucDanh] ?? 'Chưa kiểm tra';
      int statusIndex;
      charts.Color barColor;

      print(
          'Chức danh: ${chucDanh.tenChucDanh}, Mã chức danh: ${chucDanh.maChucDanh}, Status: $status');
      ...
      return ChartData(
          chucDanh.tenChucDanh ?? 'Chưa có tên', statusIndex, barColor);
    }).toList();
  }

  void _debugChartData() {
    List<ChartData> chartData = _createChartData();
    for (var data in chartData) {
      print(
          'ChartData: Tên: ${data.tenChucDanh}, Trạng thái: ${data.trangThaiIndex}, Màu: ${data.color}');
    }
  }
...
// Hàm để cắt ngắn tên chức danh nếu quá dài
  String _shortenChucDanhName(String tenChucDanh) {
    if (tenChucDanh.length > 10) {
      return '${tenChucDanh.substring(0, 32)}';
    }
    return tenChucDanh;
  }
