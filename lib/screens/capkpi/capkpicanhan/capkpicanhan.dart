...
class _CapKpiCNScreenState extends State<CapKpiCN>
  ...
  Future<void> _initializeData() async {
    try {
      ...
      await _fetchEmployeeChucDanh();
      _fetchEmployeeQuyen();
      // Lần lượt tải dữ liệu
      await _fetchTrangThaiKPI();
      await _fetchKpiCode();
      await _fetchEmployeeMaNV();

      // Sau khi có `machucdanh`, gọi phương thức sử dụng mã chức danh
      ...
      print("Mã chức quyền: $userPosition, Năm hiện tại: $currentYear");
    } catch (e) {
  ...
  Future<void> _fetchEmployeeChucDanh() async {
    try {
      final gmail = widget.userEmail;
      final maNV = await Provider.of<Usuario_provider>(context, listen: false)
          .GetMaNVByEmail(gmail);
...
// Hàm để hiển thị hộp thoại cảnh báo
  ...
  Future<void> saveChitietkpiCaNhan(
    ...
    // Kiểm tra danh sách được chọn
    debugPrint('Danh sách KPI được chọn: $selectedKpiDetails');

    // Duyệt qua danh sách KPI trong selectedKpiDetails
    for (var kpiDetail in selectedKpiDetails) {
      // In ra chi tiết KPI hiện tại
      debugPrint('Chi tiết KPI hiện tại: $kpiDetail');
      ...
      final chitietkpicanhan = ChiTietKPICaNhanNV(
        ...
        thuchien: null, // Thay vì 'null', sử dụng giá trị null thực sự
        tylehoanthanh: 0,
        ketqua: false,
      );

      debugPrint('Đối tượng ChiTietKPICaNhanNV đã tạo: $chitietkpicanhan');

      try {
        // Gửi dữ liệu qua Provider
        await Provider.of<Usuario_provider1>(context, listen: false)
            .addTieuchikpicanhan(chitietkpicanhan);
        debugPrint('Đã lưu KPI thành công cho: ${kpiDetail['makpi']}');
      } catch (e, stackTrace) {
        // In lỗi nếu xảy ra vấn đề
        debugPrint('Lỗi khi thêm mới KPI: $e');
