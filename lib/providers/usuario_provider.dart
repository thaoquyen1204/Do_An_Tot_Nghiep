...
Future<String> GetChucDanhByMaNV(String MaNV) async {
  final url = Uri.http(urlapi, 'api/NguoiDung/GetChucDanhByMaNV', {
    'MaNV': MaNV,
  });
  try {
    final resp = await http.get(url, headers: {
    ...
    if (resp.statusCode == 200) {
      String chucDanh = json.decode(resp.body); // Phản hồi là số nguyên
      return chucDanh; // Trả về giá trị trạng thái nhận được từ API
    } else if (resp.statusCode == 404) {
      return '0'; // Nếu không tìm thấy thì trả về -1
    } else {
      throw Exception('Không thể tải chi tiết KPI. Mã chức danh: ${resp.statusCode}');
    }
  } catch (e) {
    throw Exception('Đã xảy ra lỗi trong khi lấy chức danh.');
  }
}
